...
export default function Hero() {
    ...
    <section id="hero" className="min-h-screen flex items-center justify-center relative overflow-hidden">
      ...
      >
        ...
        >
          ...
          <Button
            size="lg"
            className="bg-gradient-to-r from-cyan-500 to-green-500 hover:from-cyan-600 hover:to-green-600 text-white px-8 py-3 rounded-full font-semibold shadow-lg hover:shadow-cyan-500/25 transition-all duration-300 transform hover:scale-105 glow-button"
          >
            {t("heroButton")}
          </Button>
          </a>
  ...
  {[
    { icon: Github, href: "https://github.com/RipFacuu", label: "GitHub", target: "_blank" },
    { icon: Linkedin, href: "https://www.linkedin.com/in/facundoaguirre-dev/", label: "LinkedIn", target: "_blank" },
    { icon: Mail, href: "#contact", label: "Email" },
  ].map(({ icon: Icon, href, label, target }) => (
    <motion.a
      key={label}
      href={href}
      target={target} // 👈 acá se aplica
      rel={target === "_blank" ? "noopener noreferrer" : undefined} // 👈 buena práctica de seguridad
